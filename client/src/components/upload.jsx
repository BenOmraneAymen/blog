...
export default function Upload() {
  ...
  function handleSubmit(event) {
    ...
    const urlCreate = "http://localhost:4000/blogs/create";
    const urlEdit = "http://localhost:4000/blogs/" + id;
    ...
    if (id === undefined) {
      axios.post(urlCreate, formData, config).then((response) => {
        if (response.status == 200) {
          alert("Blog uploaded successfully");
        }
      });
    } else {
      axios.put(urlEdit, formData, config).then((response) => {
        if (response.status == 200) {
  ...
  return (
    <div className="flex flex-row items-center justify-center bg-slate-100 dark:bg-slate-900 w-full h-screen">
      <form
        ...
        className="flex flex-col w-116 h-5/6 justify-evenly rounded-lg bg-white dark:bg-slate-700 border-solid border-2 border-indigo-500 dark:border-slate-200"
      >
        <h1 className="text-indigo-600 dark:text-white text-3xl mx-auto ">
          Blog Upload
        </h1>
        ...
        <div className="flex flex-col mx-auto w-100 ">
          ...
          <input
            type="text"
            className="w-100 h-12 my-3 rounded-lg border-2 dark:bg-gray-300 border-gray-300 focus:outline-none focus:border-indigo-600"
            onChange={(e) => {
            ...
            {topics.map((topic) => {
              return (
                <div className="flex flex-row items-center justify-between">
                  <div
                    className={`h-10 ${
                      selectedTopics.indexOf(topic.name) > -1
                        ? "bg-indigo-700 dark:bg-slate-500 text-white"
                        : "bg-indigo-400 dark:bg-slate-300  text-white dark:text-gray-700"
                    } px-3 py-2  rounded-md hover:bg-indigo-600 cursor-pointer m-2 `}
                    id={topic.id}
        ...
        <button
          type="submit"
          className="w-28 h-11 bg-indigo-700 dark:bg-slate-500 px-4 mx-auto rounded-md text-white"
        >
          Upload
        </button>
      </form>
